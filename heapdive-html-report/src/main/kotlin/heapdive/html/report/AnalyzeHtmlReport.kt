...
open class AnalyzeHtmlReport(protected val analysisContext: AnalysisContext, private val listProvider: ListProvider) {
    ...
    open fun analyze(progress: ProgressIndicator): HProfAnalysisReport {
        ...
        val heapSummaryReport = traverseInstanceGraph(traverseProgress)

        return HProfAnalysisReport(
                histogram = prepareHistogramSection(),
                // Heap Summary
    ...
    private fun preparePerClassSection(progress: PartialProgressIndicator): List<NominatedClass> {
        ...
        val entries = nominatedClassNames.map { className ->
    ...
    private fun prepareHistogramSection(): HistogramReport {
        ...
        return HistogramHtmlReport.prepareMergedHistogramReport(
                histogram, strongRefHistogram,
                histogramOptions, unreachableObjectsCount,
    ...
    private fun traverseInstanceGraph(progress: ProgressIndicator): HeapSummaryReport {
        ...
        while (!toVisit.isEmpty) {
            for (i in 0 until toVisit.size) {
                @Suppress("DEPRECATION") val id = toVisit[i]
        ...
        if (config.dominatorTreeOptions.includeDominatorTree) {
            ...
            if (usableDiskSpace - estimateDominatorTempFilesSize(visitedCount, edgeCount) > config.dominatorTreeOptions.diskSpaceThreshold) {
                ...
                log.append("Omitted due to low disk space\n")
            }
        }
        rootsSet.clear()
        ...
        return HeapSummaryReport(
                ...
                softBytes,
                weakBytes,
                unreachableDisposableObjects.size,
        )
    }
    ...
    private fun computeDominatorFlameGraph(nav: ObjectNavigator, rootsSet: IntSet, sizesList: IntList, edgeCount: Int): Pair<StringBuilder, DominatorFlameGraph> {
        ...
        fun signatureFor(ponum: Int): String {
            return if (ponum == rootPonum) {
                "root"
            } else {
                val key = postorderList[ponum].toLong()
                nav.getClassForObjectId(key).name.dropLastWhile { it == ';' }
            }
        }
        ...
        val metaInfo = StringBuilder().apply {
